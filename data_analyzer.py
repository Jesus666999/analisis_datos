...
def analyzeData(filepath):
    ...
    header = getHeader(data.keys())

    if (filepath == "fake_data.csv"):
        accepted_file = open("accepted_data.csv", mode = "w", newline="")
        accepted_file.write(header)    
    else:
        accepted_file = open("accepted_data.csv", mode = "a", newline="")

    accepted_writer = csv.writer(accepted_file)

    not_accepted_file = open("not_accepted_data.csv", mode = "w", newline="")
